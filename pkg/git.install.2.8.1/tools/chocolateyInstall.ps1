...
$fileType = 'exe'
$fileArgs = '/VERYSILENT /NORESTART /NOCANCEL /SP- '
$url = 'https://github.com/git-for-windows/git/releases/download/v2.8.1.windows.1/Git-2.8.1-32-bit.exe'
$url64 = 'https://github.com/git-for-windows/git/releases/download/v2.8.1.windows.1/Git-2.8.1-64-bit.exe'
...
if ($packageParameters) {
    ...
    if ($arguments.ContainsKey("NoShellExt")) {
        Write-Host "You do not want the Git shell extension"
        $fileArgs = $($fileArgs + '/COMPONENTS="icons,icons\quicklaunch,assoc,assoc_sh" /LOG')
    } else {
        $fileArgs = $($fileArgs + '/COMPONENTS="icons,icons\quicklaunch,ext,ext\shellhere,ext\guihere,assoc,assoc_sh" /LOG')
    }
} else {
